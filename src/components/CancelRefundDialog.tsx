...
export function CancelRefundDialog({
  ...
  return (
    /* The overlay scrolls and the panel is bounded. This dialog is tall — an hour
       list, a required reason and two paragraphs of refund explanation — and
       centred in a fixed overlay with no bound its top and bottom both ran off a
       short screen with nothing able to scroll to them. */
    <div className="fixed inset-0 z-[1400] flex items-center justify-center overflow-y-auto bg-black/50 p-4">
      <div className="flex max-h-[90dvh] w-full max-w-md flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-xl">
        {/* Pinned, so what is being cancelled stays readable while the body scrolls. */}
        ...
        {result ? (
          <div className="nice-scroll min-h-0 flex-1 space-y-2 overflow-y-auto px-5 pb-5 text-sm">
          ...
          <>
            {/* Only the middle scrolls, so the buttons below can never be pushed off
                a small screen — which is what made this dialog unusable. */}
            <div className="nice-scroll min-h-0 flex-1 overflow-y-auto px-5 pb-4">
              <div className="mt-4">
                ...
                {lockedCount > 0 && (
                  <p className="mt-2 rounded-lg bg-secondary/60 p-2.5 text-[11px] text-muted-foreground">
                    {lockedCount} hour{lockedCount === 1 ? "" : "s"} cannot be cancelled — that
                    court time has already passed, so it cannot be given back to anyone. The
                    remaining hours can still be released.
                  </p>
                )}
                  ...
                  <label className="mt-2 flex cursor-pointer items-start gap-2 rounded-xl border border-amber-500/40 bg-amber-500/10 p-2.5 text-[11px] text-amber-800 dark:text-amber-300">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (!e.target.checked) {
                          setSelected((prev) =>
                            prev.filter((id) => !needsOverride(states.get(id)!)),
                          );
                        }
                      }}
                    />
                    <span>
                      Also allow hours in progress or starting within {IMMINENT_MINUTES} minutes.
                      Only for maintenance, an emergency or a court closure — the player has little
                      or no notice, so message them as well.
                    </span>
                  </label>
                )}
                    ...
                    <label className="flex cursor-pointer items-start gap-2 rounded-xl border border-border p-3 text-sm has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                      ...
                      <span>
                        ...
                        <span className="mt-0.5 block text-[11px] font-normal text-muted-foreground">
                          PayMongo returns the money along the same route it arrived — the exact
                          GCash wallet, Maya account, card or bank the player paid with. You cannot
                          choose a different destination, and you never need their account details.
                          Usually lands in minutes for e-wallets; card refunds can take several
                          banking days. Recommended.
                        </span>
                      </span>
                    </label>
                    <label className="flex cursor-pointer items-start gap-2 rounded-xl border border-border p-3 text-sm has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                      ...
                      <span>
                        ...
                        <span className="mt-0.5 block text-[11px] font-normal text-muted-foreground">
                          ...
                          booking to record how and close it off. It stays “Awaiting refund” until
                          you do. Ask for an account number only — never a password, OTP or card
                          CVV.
                        </span>
                      </span>
                    </label>
            ...
            {/* Pinned footer: the decision stays one tap away at any height. */}
